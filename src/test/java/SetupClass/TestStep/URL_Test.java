...
public class URL_Test extends Set {
	...
	public void user_is_already_on_sign_up_page_cd() throws InterruptedException {
		 ...
		 * driver.findElements(By.xpath("//*[contains(text(),'Database')]"));
		 * 
		 * if (dynamicElement.size() != 0) { Assert.fail("Element is present"); } else {
		 * System.out.println("Element is not present"); } Thread.sleep(3000);
		 */
		 ...
		 * Thread.sleep(3000);
		 * 
		 * List<WebElement> dynamicElement2 =
		 * driver.findElements(By.xpath("//*[contains(text(),'Database')]"));
		 * 
		 * if (dynamicElement2.size() != 0) { System.out.println("Element is present");
